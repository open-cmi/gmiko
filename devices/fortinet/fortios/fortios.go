...
func (c *Device) connect() error {
	...
	config := &ssh.ClientConfig{
		User: c.User,
		Auth: []ssh.AuthMethod{
			ssh.Password(c.Password), // or use ssh.PublicKeys(your public key)
		},
		HostKeyCallback: ssh.InsecureIgnoreHostKey(), // use ssh.FixedHostKey(hostKey) in production env
		Config: ssh.Config{
		...
		HostKeyAlgorithms: algorithms.PublicKeyAuths,
	}
