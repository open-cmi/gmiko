...
var enablePattern *regexp.Regexp
...
func (c *Device) Enable() {
	c.writer.Write([]byte("enable\n"))
	c.ReadUntil(enablePattern, 1)
	c.writer.Write([]byte(c.Priviledge + "\n"))
	c.ReadUntil(devicePattern, 1)
}
...
func init() {
	...
	enablePattern = regexp.MustCompile(`Password:|[\w\-\(\)]+[>#]`)
}
